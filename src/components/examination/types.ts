...
export type ExaminationPage = 'overview' | 'competitions' | 'sessions' | 'candidates' | 'classes' | 'import' | 'competition-detail' | 'candidate-detail';
export type DraftDate = { day: string; month: string; year: string; planned: boolean; unknown: boolean };
export type ExaminationSession = { id: string; code: string; name: string; parent: string; organizer: string; time: string; candidates: number; national: string; nationalDate?: string; international: string; internationalDate?: string; phase: string; note: string };
