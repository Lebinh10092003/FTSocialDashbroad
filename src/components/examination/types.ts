...
export type ExaminationPage = 'overview' | 'competitions' | 'sessions' | 'candidates' | 'classes' | 'teachers' | 'import' | 'competition-detail' | 'session-detail' | 'candidate-detail' | 'class-detail' | 'teacher-detail';
...
export type Candidate = { code: string; name: string; school: string; className: string; city: string; contests: string; achievement: string; updated: string; email: string; parent: string; phone: string; identity: string; address: string; birthDate: string; sessionIds?: string[] };
export type Competition = { id: string; code: string; name: string; organizer: string; parent: string; updatedAt?: string; createdAt?: string };
...
export interface ExaminationModuleProps { onBackToWorkspace: () => void; onAccountClick: () => void; userName?: string | null; userEmail?: string | null; idToken?: string | null; googleAccessToken?: string | null; userRole: UserRole; isGuest: boolean; }
