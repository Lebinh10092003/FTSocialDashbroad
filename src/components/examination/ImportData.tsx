...
type Props = { idToken?: string | null; googleAccessToken?: string | null; canImport: boolean; onImported: (items: Candidate[]) => void };

const columns = [
  ['code', 'Mã FT'], ['name', 'Họ và tên'], ['school', 'Trường học'], ['className', 'Lớp'], ['city', 'Tỉnh/Thành phố'],
  ['contests', 'Cuộc thi'], ['achievement', 'Kết quả/Thành tích'], ['email', 'Email'], ['parent', 'Phụ huynh'],
...
const aliases: Record<string, string[]> = {
  ...
  email: ['email'], parent: ['phu huynh', 'ho ten phu huynh', 'parent'], phone: ['so dien thoai', 'sdt', 'dien thoai', 'phone'],
...
function mapRows(rawRows: ImportRow[]): Candidate[] {
  return rawRows.map((row, index) => {
    ...
    return { code, name, school: get('school'), className: get('className'), city: get('city'), contests: get('contests'), achievement: get('achievement'), email: get('email'), parent: get('parent'), phone: get('phone'), identity: get('identity'), address: get('address'), birthDate: '', updated: '' };
  }).filter(row => row.name);
}
...
export default function ImportData({ idToken, googleAccessToken, canImport, onImported }: Props) {
  ...
  const importRows = async () => {
    ...
    setLoading(true); setMessage('');
    try {
      const response = await fetch('/api/examination/import/candidates', { method: 'POST', headers, body: JSON.stringify({ records: rows, source }) });
  ...
  const downloadTemplate = () => {
    ...
    const data = [columns.map(([, label]) => label), ['FT26-0100', 'Nguyễn Minh Anh', 'THCS Cầu Giấy', '8A1', 'Hà Nội', 'IMO', 'Đạt giải Khuyến khích', 'minhanh@example.com', 'Nguyễn Thu Hà', '0988 123 456', '001212345678', 'Cầu Giấy, Hà Nội']];
  ...
  return <>
    <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-extrabold text-[#101827]">Nhập dữ liệu</h1><p className="mt-1 text-sm text-slate-600">Đọc danh sách thí sinh và kết quả từ Excel/CSV hoặc liên kết Google Sheets.</p></div><button onClick={downloadTemplate} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-[#001e40]"><Download className="h-4 w-4"/>Tải file mẫu</button></div>
