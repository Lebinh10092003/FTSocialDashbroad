...
export default function QuestionBankSettings({ idToken }: { idToken: string }) {
  ...
  return <section className="space-y-5">
    <div className="rounded-2xl border bg-white p-6 shadow-sm">
      <div className="flex items-start gap-3"><span className="rounded-xl bg-emerald-100 p-3 text-emerald-700"><Link2 className="h-6 w-6" /></span><div><p className="text-xs font-bold uppercase tracking-wider text-emerald-600">Cấu hình dùng chung</p><h2 className="mt-1 text-2xl font-extrabold text-[#001e40]">Set up ngân hàng đề thi</h2><p className="mt-2 max-w-2xl text-sm text-slate-600">Lưu liên kết rồi đồng bộ một lần. Khi tạo đề, hệ thống dùng chỉ mục đã lưu trên máy chủ thay vì tải lại Google Sheet.</p></div></div>
