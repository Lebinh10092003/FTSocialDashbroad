...
export default function TrainingAssessmentPublic({ slug, idToken = "" }: { slug: string; idToken?: string }) {
  ...
  const uploadFile = async (question: PublicQuestion, file?: File) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await apiError(response));
      const body = await response.json();
      setAttempt((current: any) => ({ ...current, uploads: [...(current?.uploads || []), body] }));
      setAnswers((current) => {
        ...
        const ids = String(practical.upload_ids || practical.upload_id || "").split(",").filter(Boolean);
        const urls = String(practical.upload_urls || practical.upload_url || "").split("\n").filter(Boolean);
        return { ...current, [question.id]: { ...practical, upload_id: String(body.id), upload_file_id: String(body.file_id || ""), upload_url: String(body.url || ""), upload_ids: [...ids, String(body.id)].join(","), upload_urls: [...urls, String(body.url || "")].join("\n") } };
      });
    } catch (error: any) {
      setMessage(String(error?.message || error));
    } finally {
  ...
  const uploadFiles = async (question: PublicQuestion, files?: FileList | File[]) => {
    for (const file of Array.from(files || [])) await uploadFile(question, file);
  };

  const deleteUpload = async (question: PublicQuestion, uploadId: number) => {
    if (!attempt?.access_token || isPreview) return;
    setBusy(true); setMessage("");
    try {
      const response = await fetch(`/api/training-assessment-attempts/${attempt.access_token}/uploads/${uploadId}`, { method: "DELETE" });
      if (!response.ok) throw new Error(await apiError(response));
      const remaining = (attempt.uploads || []).filter((item: any) => Number(item.id) !== Number(uploadId));
      setAttempt((current: any) => ({ ...current, uploads: (current?.uploads || []).filter((item: any) => Number(item.id) !== Number(uploadId)) }));
      setAnswers((current) => {
        const existing = current[question.id];
        const practical = existing && typeof existing === "object" && !Array.isArray(existing) ? existing : { link: typeof existing === "string" ? existing : "" };
        const questionUploads = remaining.filter((item: any) => String(item.question_id) === String(question.id));
        const last = questionUploads.at(-1);
        return { ...current, [question.id]: { ...practical, upload_id: String(last?.id || ""), upload_file_id: String(last?.file_id || ""), upload_url: String(last?.url || ""), upload_ids: questionUploads.map((item: any) => item.id).join(","), upload_urls: questionUploads.map((item: any) => item.url).filter(Boolean).join("\n") } };
      });
    } catch (error: any) { setMessage(String(error?.message || error)); } finally { setBusy(false); }
  };
  ...
  const currentQuestionUploads = (attempt?.uploads || []).filter((item: any) => String(item.question_id) === String(currentQuestion?.id));
    ...
    <div lang="vi" translate="no" className="notranslate min-h-screen bg-slate-100 pb-28 text-slate-900">
      ...
      <main className="mx-auto grid w-[92vw] max-w-[1800px] gap-5 px-0 py-4 sm:py-6 md:grid-cols-[220px_minmax(0,1fr)] lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        <section className="min-w-0">{isPreview && <div className="mb-4 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900"><b>Chế độ xem trước của người tạo.</b> Đây là giao diện người trả lời để kiểm tra trải nghiệm; mọi thao tác chỉ là mô phỏng, không tạo lượt làm, không lưu tệp và không thể nộp bài.</div>}{message && <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{message}</div>}{reviewMode ? <article className="rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
          ...
          {(currentQuestion.type === "practical_submission" || currentQuestion.type === "file_upload") && (() => { const stored = answers[currentQuestion.id]; const practical = stored && typeof stored === "object" && !Array.isArray(stored) ? stored as Record<string, string> : { link: typeof stored === "string" ? stored : "" }; return isNotebookLmImageSubmission ? <div className="mt-6 grid gap-4"><p className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">Câu thực hành NotebookLM: có thể nộp <b>một hoặc nhiều ảnh minh chứng</b>. Không cần dán link sản phẩm.</p>{currentQuestionUploads.length > 0 && <div className="grid gap-3 sm:grid-cols-2">{currentQuestionUploads.map((upload: any, index: number) => <figure key={upload.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><a href={upload.url} target="_blank" rel="noreferrer" className="block bg-slate-50"><img src={upload.url} alt={`Ảnh minh chứng ${index + 1}`} className="h-52 w-full object-contain" /></a><figcaption className="flex items-center justify-between gap-3 border-t p-3"><span className="min-w-0 truncate text-xs font-bold text-slate-700">Ảnh {index + 1}: {upload.name}</span><button type="button" disabled={busy} onClick={() => void deleteUpload(currentQuestion, upload.id)} className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-rose-200 px-2 py-1 text-xs font-bold text-rose-700 hover:bg-rose-50 disabled:opacity-50"><Trash2 className="h-3.5 w-3.5" />Xóa</button></figcaption></figure>)}</div>}<label onDragOver={(event) => { event.preventDefault(); event.currentTarget.classList.add("border-blue-500", "bg-blue-50"); }} onDragLeave={(event) => event.currentTarget.classList.remove("border-blue-500", "bg-blue-50")} onDrop={(event) => { event.preventDefault(); event.currentTarget.classList.remove("border-blue-500", "bg-blue-50"); void uploadFiles(currentQuestion, event.dataTransfer.files); }} className="flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-slate-300 p-6 text-center transition hover:border-blue-400 hover:bg-blue-50"><FileUp className="h-8 w-8 text-blue-600" /><b className="mt-2">{currentQuestionUploads.length ? "Thêm ảnh minh chứng khác" : "Thả ảnh vào đây hoặc bấm để chọn"}</b><span className="mt-1 text-xs text-slate-500">Có thể chọn nhiều ảnh · JPG, PNG, WEBP · tối đa 5 MB/ảnh</span><input type="file" multiple accept="image/*" className="hidden" onChange={(event) => { void uploadFiles(currentQuestion, event.target.files || undefined); event.currentTarget.value = ""; }} /></label></div> : <div className="mt-6 grid gap-3"><p className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-700">Câu thực hành này chỉ nhận <b>link chia sẻ</b>; không cần tải ảnh hoặc tệp lên.</p><label><span className="mb-1 block text-sm font-bold">Link sản phẩm / thư mục Drive</span><div className="relative"><Link2 className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input type="url" className="ft-input pl-10" value={practical.link || ""} onChange={(event) => setAnswer(currentQuestion.id, { link: event.target.value })} placeholder="https://drive.google.com/..." /></div></label></div>; })()}
