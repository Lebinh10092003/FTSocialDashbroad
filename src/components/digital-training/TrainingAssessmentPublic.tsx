    ...
    <div className="min-h-screen bg-slate-100 pb-28 text-slate-900">
      ...
      <main className="mx-auto grid max-w-6xl gap-5 px-4 py-6 lg:grid-cols-[230px_minmax(0,1fr)]">
        <aside className="h-fit rounded-2xl border bg-white p-4 shadow-sm lg:sticky lg:top-24">
          ...
          <div className="mt-4 space-y-2 border-t pt-3 text-[11px] text-slate-500"><p><span className="mr-2 inline-block h-2.5 w-2.5 rounded bg-emerald-100" />Đã trả lời</p><p><Bookmark className="mr-1 inline h-3 w-3 fill-amber-400 text-amber-500" />Đánh dấu xem lại</p></div>
        </aside>
        <section className="min-w-0">{message && <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-sm text-rose-700">{message}</div>}{currentQuestion && <article className="rounded-2xl border bg-white p-5 shadow-sm sm:p-7">
          ...
          {(currentQuestion.type === "practical_submission" || currentQuestion.type === "file_upload") && (() => { const stored = answers[currentQuestion.id]; const practical = stored && typeof stored === "object" && !Array.isArray(stored) ? stored as Record<string, string> : { link: typeof stored === "string" ? stored : "" }; return <div className="mt-6 grid gap-4"><label><span className="mb-1 block text-sm font-bold">Link sản phẩm / thư mục Drive</span><div className="relative"><Link2 className="absolute left-3 top-3 h-4 w-4 text-slate-400" /><input type="url" className="ft-input pl-10" value={practical.link || ""} onChange={(event) => setAnswer(currentQuestion.id, { ...practical, link: event.target.value })} placeholder="https://drive.google.com/..." /></div></label><label className="flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-slate-300 p-6 text-center hover:border-blue-400 hover:bg-blue-50"><FileImage className="h-8 w-8 text-blue-600" /><b className="mt-2">{practical.upload_id ? "Đã tải ảnh minh chứng" : "Tải ảnh minh chứng"}</b><span className="mt-1 text-xs text-slate-500">JPG, PNG, WEBP - tối đa 5 MB</span><input type="file" accept="image/*" className="hidden" onChange={(event) => void uploadImage(currentQuestion, event.target.files?.[0])} /></label>{practical.upload_url && <a href={practical.upload_url} target="_blank" rel="noreferrer" className="text-sm font-bold text-blue-700 underline">Xem ảnh đã tải</a>}</div>; })()}
      ...
      <footer className="fixed inset-x-0 bottom-0 z-20 border-t bg-white p-4 shadow-[0_-8px_24px_rgba(15,23,42,.08)]"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4"><p className="text-sm text-slate-600">{unansweredRequired ? `Còn ${unansweredRequired} câu bắt buộc chưa trả lời` : "Đã trả lời đủ các câu bắt buộc"}</p><button disabled={busy || unansweredRequired > 0} onClick={async () => { const confirmed = await appDialog.confirm("Nộp bài ngay? Sau khi nộp bạn không thể sửa câu trả lời.", { title: "Xác nhận nộp bài", confirmText: "Nộp bài", tone: "warning" }); if (confirmed) void save(true); }} className="ft-primary disabled:cursor-not-allowed disabled:opacity-50"><Send className="h-4 w-4" />Nộp bài</button></div></footer>
    </div>
  );
}
