...
function AssessmentEvidenceImage({ source, auth, alt }: { source: string; auth: Record<string, string>; alt: string }) {
  ...
  return <img src={previewUrl} alt={alt} className="mt-3 max-h-[38rem] w-full rounded-lg border bg-white object-contain" />;
}
...
export default function TrainingAssessmentsAdmin({
  ...
  const publicLink = selected
    ...
    : "";
  const adminDetailLink = selected
    ? `${window.location.origin}${assessmentDetailPath(selected.id)}`
    : "";
  ...
  const openGrading = async (attempt?: any) => {
    ...
    setQuestionScores(Object.fromEntries(Object.entries(candidate.grading || {}).map(([questionId, score]) => [questionId, String(score)])));
  ...
  const saveQuestionGrades = async () => {
    ...
    try {
      ...
      const completed = refreshed.filter((item) => item.status !== "in_progress");
      const currentIndex = completed.findIndex((item) => item.id === updated.id);
      const next = completed[currentIndex + 1];
      setResults(refreshed);
      if (next) {
        setGradingAttempt(next);
        setQuestionScores(Object.fromEntries(Object.entries(next.grading || {}).map(([questionId, score]) => [questionId, String(score)])));
        setActiveGradingQuestionId("");
        setNotice("Đã lưu điểm. Đang chuyển sang bài làm tiếp theo.");
      } else {
        setGradingAttempt(updated);
        setNotice(updated.manual_grading_required ? "Đã lưu điểm tạm. Vẫn còn câu cần chấm." : "Đã lưu điểm bài làm. Đây là bài cuối cùng.");
      }
    } catch (error: any) {
      setNotice(String(error?.message || error));
    } finally {
  ...
  if (screen === "grading" && selected && gradingAttempt) {
    ...
    const activeQuestion = gradingQuestions.find((question) => String(question.id) === activeGradingQuestionId);
    return <section className="mt-6 pb-8"><main className="mx-auto max-w-5xl"><button onClick={() => setScreen("detail")} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-slate-600"><ArrowLeft className="h-4 w-4" />Quay lại trang bài kiểm tra</button><div className="rounded-2xl border border-violet-200 bg-violet-50 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-violet-700">Chế độ chấm bài</p><h1 className="mt-1 text-xl font-extrabold text-[#001e40]">{selected.title}</h1><p className="mt-1 text-sm text-slate-600">Bài {gradingIndex + 1}/{gradingCandidates.length} · {gradingAttempt.variant}</p></div><div className="text-right text-sm"><b>{gradingAttempt.respondent_name}</b><p className="mt-1 text-slate-600">{[gradingAttempt.position, gradingAttempt.organization].filter(Boolean).join(" · ") || "Chưa khai báo chức vụ / đơn vị"}</p><p className="mt-1 text-slate-600">{gradingAttempt.email || gradingAttempt.phone || "Không có liên hệ"}</p></div></div></div><div className="mt-5 rounded-2xl border bg-white p-4"><p className="text-sm font-bold text-slate-800">Thông tin học viên</p><div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><p><b>Mã đề:</b> {gradingAttempt.variant}</p><p><b>Trạng thái:</b> {gradingAttempt.status === "submitted" ? "Đã nộp" : "Hết giờ"}</p><p><b>Tổng điểm:</b> {Number(gradingAttempt.score || 0).toLocaleString("vi-VN")} / {Number(gradingAttempt.max_score || 0).toLocaleString("vi-VN")}</p></div></div><div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Lưu ý khi sửa điểm trắc nghiệm:</b> hệ thống vẫn cho phép ghi đè điểm tự động; điểm sửa sẽ là điểm chính thức sau khi lưu.</div><div className="mt-5 rounded-2xl border bg-white p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Câu hỏi của bài làm</p><p className="mt-1 text-sm text-slate-500">Bấm vào một câu để mở chi tiết bài làm và chấm điểm.</p></div><span className="text-sm font-bold text-slate-600">{gradingQuestions.length} câu</span></div><div className="mt-4 space-y-2">{gradingQuestions.map((question, index) => { const selectedQuestion = String(question.id) === activeGradingQuestionId; const enteredScore = questionScores[question.id]; const automatic = ["single_choice", "multiple_choice"].includes(question.type) || (["short_answer", "matching", "ordering"].includes(question.type) && (question.correct_answers || []).length > 0); return <button key={question.id} onClick={() => setActiveGradingQuestionId(selectedQuestion ? "" : String(question.id))} className={`flex w-full items-center justify-between gap-3 rounded-xl border px-4 py-3 text-left transition ${selectedQuestion ? "border-blue-500 bg-blue-50" : "hover:border-blue-300 hover:bg-slate-50"}`}><span><b className="block text-sm text-slate-900">{question.order || index + 1}. {question.text}</b><span className="mt-1 block text-xs text-slate-500">{question.question_code || questionTypeLabels[question.type] || question.type} · {question.points} điểm</span></span><span className={`shrink-0 rounded-full px-2 py-1 text-xs font-bold ${enteredScore !== undefined && enteredScore !== "" ? "bg-emerald-100 text-emerald-800" : automatic ? "bg-blue-100 text-blue-800" : "bg-amber-100 text-amber-800"}`}>{enteredScore !== undefined && enteredScore !== "" ? `${enteredScore}/${question.points}` : automatic ? "Điểm tự động" : "Chưa chấm"}</span></button>; })}</div></div>{activeQuestion ? (() => { const question = activeQuestion; const questionIndex = gradingQuestions.findIndex((item) => String(item.id) === String(question.id)); const answer = gradingAttempt.answers?.[question.id]; const upload = (gradingAttempt.uploads || []).find((item: any) => String(item.question_id) === String(question.id)); const answerLink = typeof answer === "object" && answer ? String(answer.link || answer.upload_url || upload?.url || "") : ""; const isImage = /^image\//.test(String(upload?.content_type || "")) || /\.(png|jpe?g|gif|webp)(?:\?|$)/i.test(String(upload?.name || answerLink)); const answerText = Array.isArray(answer) ? answer.join(", ") : typeof answer === "object" && answer ? Object.entries(answer).filter(([key]) => !["link", "upload_id", "upload_file_id", "upload_url"].includes(key)).map(([key, value]) => `${key}: ${value}`).join(" · ") : String(answer || ""); const automatic = ["single_choice", "multiple_choice"].includes(question.type) || (["short_answer", "matching", "ordering"].includes(question.type) && (question.correct_answers || []).length > 0); const evidenceUrl = upload?.id ? `/api/digital-training/assessments/${selected.id}/results/${gradingAttempt.id}/uploads/${upload.id}/content` : ""; return <article className="mt-5 rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-700">{question.question_code || `Câu ${questionIndex + 1}`}</p><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{questionTypeLabels[question.type] || question.type}</span><span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">{question.points} điểm</span></div></div><label className="min-w-36"><span className={`mb-1 block text-xs font-bold ${automatic ? "text-amber-800" : "text-emerald-800"}`}>{automatic ? "Điểm chính thức · ghi đè tự động" : "Điểm chấm trực tiếp"}</span><div className="flex items-center gap-2"><input type="number" min="0" max={Number(question.points || 0)} step="0.25" value={questionScores[question.id] ?? ""} placeholder={automatic ? "Tự động" : "Chưa chấm"} onChange={(event) => setQuestionScores((current) => ({ ...current, [question.id]: event.target.value }))} className="w-24 rounded-lg border px-2 py-1.5 text-sm" /><span className="text-xs text-slate-500">/ {question.points}</span></div></label></div><h2 className="mt-4 whitespace-pre-wrap text-lg font-bold leading-7 text-slate-900">{question.order}. {question.text}</h2>{question.media_url && <a href={question.media_url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-bold text-blue-700 underline">Mở tư liệu minh họa</a>}{(question.options || []).length > 0 && <ol className="mt-4 space-y-2">{question.options.map((option: any) => <li key={option.key} className="rounded-lg border bg-slate-50 px-3 py-2 text-sm"><b>{option.key}.</b> {option.text}{option.match_text ? ` → ${option.match_text}` : ""}</li>)}</ol>}<div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Bài làm của học viên</p>{isImage && evidenceUrl ? <><AssessmentEvidenceImage source={evidenceUrl} auth={auth} alt={`Bài nộp câu ${question.order}`} />{answerLink && <a href={answerLink} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm font-bold text-blue-700 underline">Mở tệp gốc</a>}</> : answerLink ? <a href={answerLink} target="_blank" rel="noreferrer" className="mt-3 inline-block break-all font-bold text-blue-700 underline">{answerLink}</a> : <p className="mt-3 whitespace-pre-wrap text-sm text-slate-800">{answerText || "Chưa trả lời / chưa nộp bài."}</p>}</div><div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900"><b>Đáp án chuẩn:</b> {(question.correct_answers || []).join(", ") || "Chấm thủ công"}</div></article>; })() : <div className="mt-5 rounded-2xl border border-dashed bg-slate-50 p-8 text-center text-sm text-slate-500">Chọn một câu ở danh sách phía trên để xem chi tiết bài làm.</div>}{notice && <p className={`mt-4 rounded-xl p-3 text-sm ${notice.startsWith("Đã") ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-700"}`}>{notice}</p>}<div className="mt-6 flex flex-wrap items-center justify-between gap-2 rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap gap-2"><button onClick={() => setShowGradingRoster(true)} className="ft-btn ft-btn-secondary">Danh sách thí sinh</button><button disabled={!previousAttempt || busy} onClick={() => void openGrading(previousAttempt)} className="ft-btn ft-btn-secondary">Bài làm trước</button><button disabled={!nextAttempt || busy} onClick={() => void openGrading(nextAttempt)} className="ft-btn ft-btn-secondary">Bài làm tiếp theo</button></div><button disabled={busy} onClick={() => void saveQuestionGrades()} className="ft-primary">{busy ? "Đang lưu..." : "Lưu điểm"}</button></div>{showGradingRoster && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4"><div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase text-violet-700">Danh sách thí sinh</p><h2 className="mt-1 text-xl font-extrabold">Chọn bài làm cần chấm</h2></div><button onClick={() => setShowGradingRoster(false)} className="ft-btn ft-btn-secondary">Đóng</button></div><div className="mt-4 space-y-2">{gradingCandidates.map((item, index) => <button key={item.id} onClick={() => { setShowGradingRoster(false); void openGrading(item); }} className={`flex w-full items-center justify-between gap-3 rounded-xl border p-4 text-left ${item.id === gradingAttempt.id ? "border-violet-400 bg-violet-50" : "hover:border-violet-300"}`}><span><b className="block">{index + 1}. {item.respondent_name}</b><span className="text-sm text-slate-600">{item.variant} · {[item.position, item.organization].filter(Boolean).join(" · ") || "Chưa có thông tin đơn vị"}</span></span><span className={`rounded-full px-2 py-1 text-xs font-bold ${item.manual_grading_required ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{item.manual_grading_required ? "Chưa chấm xong" : "Đã chấm"}</span></button>)}</div></div></div>}</main></section>;
    return <section className="mt-6 pb-8"><main className="mx-auto max-w-5xl"><button onClick={() => setScreen("detail")} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-slate-600"><ArrowLeft className="h-4 w-4" />Quay lại trang bài kiểm tra</button><div className="mb-5 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-violet-200 bg-violet-50 p-4"><div><p className="text-xs font-bold uppercase tracking-wide text-violet-700">Chế độ chấm bài</p><h1 className="mt-1 text-xl font-extrabold text-[#001e40]">{selected.title}</h1><p className="mt-1 text-sm text-slate-600">Bài {gradingIndex + 1}/{gradingCandidates.length} · {gradingAttempt.variant}</p></div><div className="text-right text-sm"><b>{gradingAttempt.respondent_name}</b><p className="mt-1 text-slate-600">{[gradingAttempt.position, gradingAttempt.organization].filter(Boolean).join(" · ") || "Chưa khai báo chức vụ / đơn vị"}</p><p className="mt-1 text-slate-600">{gradingAttempt.email || gradingAttempt.phone || "Không có liên hệ"}</p></div></div><div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Lưu ý:</b> có thể sửa điểm trắc nghiệm, nhưng đây là ghi đè điểm tự động và sẽ là điểm chính thức sau khi lưu.</div><div className="space-y-4">{gradingQuestions.map((question, index) => { const answer = gradingAttempt.answers?.[question.id]; const upload = (gradingAttempt.uploads || []).find((item: any) => String(item.question_id) === String(question.id)); const answerLink = typeof answer === "object" && answer ? String(answer.link || answer.upload_url || upload?.url || "") : ""; const imageUrl = typeof answer === "object" && answer ? String(answer.upload_url || upload?.url || "") : String(upload?.url || ""); const isImage = /^image\//.test(String(upload?.content_type || "")) || /\.(png|jpe?g|gif|webp)(?:\?|$)/i.test(imageUrl); const answerText = Array.isArray(answer) ? answer.join(", ") : typeof answer === "object" && answer ? Object.entries(answer).filter(([key]) => !["link", "upload_id", "upload_file_id", "upload_url"].includes(key)).map(([key, value]) => `${key}: ${value}`).join(" · ") : String(answer || ""); const automatic = ["single_choice", "multiple_choice"].includes(question.type) || (["short_answer", "matching", "ordering"].includes(question.type) && (question.correct_answers || []).length > 0); return <article key={question.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-700">{question.question_code || `Câu ${index + 1}`}</p><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{questionTypeLabels[question.type] || question.type}</span>{question.category && <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">{question.category}</span>}<span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">{question.points} điểm</span></div></div><label className="min-w-36"><span className={`mb-1 block text-xs font-bold ${automatic ? "text-amber-800" : "text-emerald-800"}`}>{automatic ? "Điểm chính thức · ghi đè tự động" : "Điểm chấm trực tiếp"}</span><div className="flex items-center gap-2"><input type="number" min="0" max={Number(question.points || 0)} step="0.25" value={questionScores[question.id] ?? ""} placeholder={automatic ? "Tự động" : "Chưa chấm"} onChange={(event) => setQuestionScores((current) => ({ ...current, [question.id]: event.target.value }))} className="w-24 rounded-lg border px-2 py-1.5 text-sm" /><span className="text-xs text-slate-500">/ {question.points}</span></div></label></div><h2 className="mt-4 whitespace-pre-wrap text-lg font-bold leading-7 text-slate-900">{question.order}. {question.text}</h2>{question.media_url && <a href={question.media_url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-bold text-blue-700 underline">Mở tư liệu minh họa</a>}{(question.options || []).length > 0 && <ol className="mt-4 space-y-2">{question.options.map((option: any) => <li key={option.key} className="rounded-lg border bg-slate-50 px-3 py-2 text-sm"><b>{option.key}.</b> {option.text}{option.match_text ? ` → ${option.match_text}` : ""}</li>)}</ol>}<div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4"><p className="text-xs font-bold uppercase tracking-wide text-blue-700">Bài làm của học viên</p>{isImage && imageUrl ? <><img src={imageUrl} alt={`Bài nộp câu ${question.order}`} className="mt-3 max-h-[38rem] w-full rounded-lg border bg-white object-contain" /><a href={imageUrl} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm font-bold text-blue-700 underline">Mở ảnh gốc</a></> : answerLink ? <a href={answerLink} target="_blank" rel="noreferrer" className="mt-3 inline-block break-all font-bold text-blue-700 underline">{answerLink}</a> : <p className="mt-3 whitespace-pre-wrap text-sm text-slate-800">{answerText || "Chưa trả lời / chưa nộp bài."}</p>}</div><div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900"><b>Đáp án chuẩn:</b> {(question.correct_answers || []).join(", ") || "Chấm thủ công"}</div></article>; })}</div>{notice && <p className={`mt-4 rounded-xl p-3 text-sm ${notice.startsWith("Đã") ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-700"}`}>{notice}</p>}<div className="mt-6 flex flex-wrap items-center justify-between gap-2 rounded-2xl border bg-white p-4 shadow-sm"><div className="flex flex-wrap gap-2"><button onClick={() => setShowGradingRoster(true)} className="ft-btn ft-btn-secondary">Danh sách thí sinh</button><button disabled={!previousAttempt || busy} onClick={() => void openGrading(previousAttempt)} className="ft-btn ft-btn-secondary">Bài làm trước</button><button disabled={!nextAttempt || busy} onClick={() => void openGrading(nextAttempt)} className="ft-btn ft-btn-secondary">Bài làm tiếp theo</button></div><button disabled={busy} onClick={() => void saveQuestionGrades()} className="ft-primary">{busy ? "Đang lưu..." : "Lưu điểm"}</button></div>{showGradingRoster && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/35 p-4"><div className="max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase text-violet-700">Danh sách thí sinh</p><h2 className="mt-1 text-xl font-extrabold">Chọn bài làm cần chấm</h2></div><button onClick={() => setShowGradingRoster(false)} className="ft-btn ft-btn-secondary">Đóng</button></div><div className="mt-4 space-y-2">{gradingCandidates.map((item, index) => <button key={item.id} onClick={() => { setShowGradingRoster(false); void openGrading(item); }} className={`flex w-full items-center justify-between gap-3 rounded-xl border p-4 text-left ${item.id === gradingAttempt.id ? "border-violet-400 bg-violet-50" : "hover:border-violet-300"}`}><span><b className="block">{index + 1}. {item.respondent_name}</b><span className="text-sm text-slate-600">{item.variant} · {[item.position, item.organization].filter(Boolean).join(" · ") || "Chưa có thông tin đơn vị"}</span></span><span className={`rounded-full px-2 py-1 text-xs font-bold ${item.manual_grading_required ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>{item.manual_grading_required ? "Chưa chấm xong" : "Đã chấm"}</span></button>)}</div></div></div>}</main></section>;
    return <section className="mt-6 space-y-5"><button onClick={() => setScreen("detail")} className="inline-flex items-center gap-2 text-sm font-bold text-slate-600"><ArrowLeft className="h-4 w-4" />Quay lại trang bài kiểm tra</button><div className="rounded-2xl border border-violet-200 bg-violet-50 p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-wide text-violet-700">Chế độ chấm bài</p><h2 className="mt-1 text-2xl font-extrabold text-[#001e40]">{selected.title}</h2><p className="mt-1 text-sm text-slate-600">Hiển thị toàn bộ bài làm; điểm từng câu được lưu trực tiếp vào hệ thống.</p></div><div className="rounded-xl border border-violet-200 bg-white px-4 py-3 text-sm"><b>{gradingAttempt.respondent_name}</b><p className="mt-1 text-slate-600">{[gradingAttempt.position, gradingAttempt.organization].filter(Boolean).join(" · ") || "Chưa khai báo chức vụ / đơn vị"}</p><p className="mt-1 text-slate-600">{gradingAttempt.email || gradingAttempt.phone || "Không có liên hệ"}</p></div></div></div><div className="rounded-2xl border bg-white p-4"><label className="block text-sm font-bold text-slate-800">Thông tin học viên<select value={gradingAttempt.id} onChange={(event) => { const next = results.find((item) => String(item.id) === event.target.value); if (next) void openGrading(next); }} className="ft-input mt-2">{results.filter((item) => item.status !== "in_progress").map((item) => <option key={item.id} value={item.id}>{item.respondent_name} · {item.variant} · {item.manual_grading_required ? "Chưa chấm xong" : "Đã chấm"}</option>)}</select></label><div className="mt-3 grid gap-2 text-sm sm:grid-cols-3"><p><b>Mã đề:</b> {gradingAttempt.variant}</p><p><b>Trạng thái:</b> {gradingAttempt.status === "submitted" ? "Đã nộp" : "Hết giờ"}</p><p><b>Tổng điểm:</b> {Number(gradingAttempt.score || 0).toLocaleString("vi-VN")} / {Number(gradingAttempt.max_score || 0).toLocaleString("vi-VN")}</p></div></div><div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><b>Lưu ý khi sửa điểm trắc nghiệm:</b> hệ thống vẫn cho phép ghi đè điểm tự động, nhưng điểm sửa sẽ là điểm chính thức và được đồng bộ ra Sheet.</div><div className="space-y-4">{gradingQuestions.map((question, index) => { const answer = gradingAttempt.answers?.[question.id]; const upload = (gradingAttempt.uploads || []).find((item: any) => String(item.question_id) === String(question.id)); const answerLink = typeof answer === "object" && answer ? String(answer.link || answer.upload_url || upload?.url || "") : ""; const imageUrl = typeof answer === "object" && answer ? String(answer.upload_url || upload?.url || "") : String(upload?.url || ""); const isImage = /^image\//.test(String(upload?.content_type || "")) || /\.(png|jpe?g|gif|webp)(?:\?|$)/i.test(imageUrl); const answerText = Array.isArray(answer) ? answer.join(", ") : typeof answer === "object" && answer ? Object.entries(answer).filter(([key]) => !["link", "upload_id", "upload_file_id", "upload_url"].includes(key)).map(([key, value]) => `${key}: ${value}`).join(" · ") : String(answer || ""); const automatic = ["single_choice", "multiple_choice"].includes(question.type) || (["short_answer", "matching", "ordering"].includes(question.type) && (question.correct_answers || []).length > 0); return <article key={question.id} className="rounded-2xl border bg-white p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-blue-700">{question.question_code || `Câu ${index + 1}`}</p><div className="mt-2 flex flex-wrap gap-2"><span className="rounded-full bg-slate-100 px-2 py-1 text-xs font-bold">{questionTypeLabels[question.type] || question.type}</span><span className="rounded-full bg-amber-50 px-2 py-1 text-xs font-bold text-amber-800">{question.points} điểm</span>{automatic && <span className="rounded-full bg-blue-50 px-2 py-1 text-xs font-bold text-blue-700">Có điểm tự động</span>}</div></div><label className="min-w-40"><span className={`mb-1 block text-xs font-bold ${automatic ? "text-amber-800" : "text-emerald-800"}`}>{automatic ? "Điểm chính thức · sửa điểm tự động" : "Điểm chấm trực tiếp"}</span><div className="flex items-center gap-2"><input type="number" min="0" max={Number(question.points || 0)} step="0.25" value={questionScores[question.id] ?? ""} placeholder={automatic ? "Tự động" : "Chưa chấm"} onChange={(event) => setQuestionScores((current) => ({ ...current, [question.id]: event.target.value }))} className="w-24 rounded-lg border px-2 py-1.5 text-sm" /><span className="text-xs text-slate-500">/ {question.points}</span></div></label></div><h3 className="mt-4 whitespace-pre-wrap text-lg font-bold leading-7 text-slate-900">{question.order}. {question.text}</h3>{question.media_url && <a href={question.media_url} target="_blank" rel="noreferrer" className="mt-3 inline-block text-sm font-bold text-blue-700 underline">Mở tư liệu của câu hỏi</a>}<div className="mt-4 grid gap-4 lg:grid-cols-2"><section className="rounded-xl border border-slate-200 bg-slate-50 p-4"><p className="text-xs font-bold uppercase text-slate-500">Bài làm của học viên</p>{isImage && imageUrl ? <img src={imageUrl} alt={`Bài nộp câu ${question.order}`} className="mt-3 max-h-[32rem] w-full rounded-lg border bg-white object-contain" /> : answerLink ? <a href={answerLink} target="_blank" rel="noreferrer" className="mt-3 inline-block break-all font-bold text-blue-700 underline">{answerLink}</a> : <p className="mt-3 whitespace-pre-wrap text-sm text-slate-800">{answerText || "Chưa trả lời / chưa nộp bài."}</p>}{isImage && answerLink && <a href={answerLink} target="_blank" rel="noreferrer" className="mt-2 inline-block text-sm font-bold text-blue-700 underline">Mở tệp gốc</a>}</section><section className="rounded-xl border border-emerald-200 bg-emerald-50 p-4"><p className="text-xs font-bold uppercase text-emerald-700">Đáp án / cấu hình chấm</p>{(question.options || []).length > 0 && <ol className="mt-3 space-y-2">{question.options.map((option: any) => <li key={option.key} className="rounded-lg border border-emerald-100 bg-white px-3 py-2 text-sm"><b>{option.key}.</b> {option.text}{option.match_text ? ` → ${option.match_text}` : ""}</li>)}</ol>}<p className="mt-3 text-sm text-emerald-900"><b>Đáp án chuẩn:</b> {(question.correct_answers || []).join(", ") || "Chấm thủ công"}</p></section></div></article>; })}</div>{notice && <p className={`rounded-xl p-3 text-sm ${notice.startsWith("Đã") ? "bg-emerald-50 text-emerald-800" : "bg-rose-50 text-rose-700"}`}>{notice}</p>}<div className="sticky bottom-4 flex justify-end rounded-2xl border bg-white/95 p-4 shadow-lg"><button disabled={busy} onClick={() => void saveQuestionGrades()} className="ft-primary">{busy ? "Đang lưu..." : "Lưu điểm bài làm"}</button></div></section>;
  }
    ...
    return (
      <section className="mt-6 space-y-5">
        ...
        <article className="assessment-detail-card overflow-hidden rounded-2xl border bg-white shadow-sm">
          ...
          <div className={`${detailTab === "overview" ? "grid" : "hidden"} gap-5 border-t bg-slate-50 p-6 lg:grid-cols-[minmax(0,1fr)_220px]`}>
            <div>
              ...
              <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4"><p className="text-sm font-extrabold text-slate-900">Xem trước bài kiểm tra</p><p className="mt-1 text-xs text-slate-500">Dùng nút Xem trước ở thanh điều khiển để mở mặc định chế độ quản trị viên. Trong màn hình đó có thể chuyển sang chế độ học viên.</p></section>
            </div>
        ...
        <div className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <div className="flex items-center justify-between gap-3 p-5"><div><h3 className="text-lg font-extrabold">Kết quả người học</h3><p className="text-sm text-slate-500">Chọn bút chì để mở bài làm và chấm/sửa điểm theo từng câu.</p></div><button disabled={!results.length} onClick={exportResults} className="ft-btn ft-btn-secondary"><Download className="h-4 w-4" />Xuất XLSX</button></div>
          <div className="overflow-x-auto"><table className="ft-table min-w-[1260px]"><thead><tr><th>STT</th><th>Người học</th><th>Liên hệ</th><th>Tổ chuyên môn/Phòng ban</th><th>Chức vụ</th><th>Mã đề</th><th>Điểm</th><th>Trạng thái</th><th>Bắt đầu lúc</th><th>Nộp lúc</th><th aria-label="Thao tác" /></tr></thead><tbody>{results.length ? results.map((item, index) => <tr key={item.id}><td>{index + 1}</td><td><b>{item.respondent_name}</b></td><td>{item.email || item.phone || "—"}</td><td>{item.organization || "—"}</td><td>{item.position || "—"}</td><td>{item.variant}</td><td><b>{Number(item.score || 0).toLocaleString("vi-VN")} / {Number(item.max_score || 0).toLocaleString("vi-VN")}</b><span className={`mt-1 block text-[11px] font-bold ${item.manual_grading_required ? "text-amber-800" : "text-emerald-700"}`}>{item.manual_grading_required ? "Cần chấm" : "Đã chấm"}</span></td><td>{item.status === "submitted" ? "Đã nộp" : item.status === "timed_out" ? "Hết giờ" : "Đang làm"}<div className={`mt-2 text-[11px] font-bold ${item.sync_status === "synced" ? "text-emerald-600" : item.sync_status === "error" ? "text-rose-600" : "text-amber-600"}`}>Sync: {item.sync_status || "pending"}</div>{item.sync_error && <p className="mt-1 max-w-52 text-[10px] text-rose-600" title={item.sync_error}>Lỗi: {item.sync_error}</p>}{item.status === "in_progress" && <button disabled={busy} onClick={() => void endAttempt(item)} className="mt-2 rounded border border-amber-300 px-2 py-1 text-[11px] font-bold text-amber-800">Kết thúc lượt</button>}{item.status !== "in_progress" && item.sync_status !== "synced" && <button disabled={busy} onClick={() => void updateResultStorage(item)} className="mt-2 rounded border px-2 py-1 text-[11px] font-bold">Thử lại</button>}</td><td>{item.started_at ? new Date(item.started_at).toLocaleString("vi-VN") : "—"}</td><td>{item.submitted_at ? new Date(item.submitted_at).toLocaleString("vi-VN") : "—"}</td><td><div className="flex items-center gap-2"><button disabled={busy || item.status === "in_progress"} onClick={() => void openGrading(item)} className="rounded-lg border border-blue-200 p-2 text-blue-700 hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-45" title={item.status === "in_progress" ? "Chỉ chấm bài đã kết thúc" : selected.status === "closed" ? "Mở chấm bài / sửa điểm" : "Đóng bài kiểm tra trước khi chấm"} aria-label={`Chấm hoặc sửa bài làm của ${item.respondent_name}`}><Pencil className="h-4 w-4" /></button><button disabled={busy} onClick={() => void updateResultStorage(item, true)} className="rounded-lg border border-rose-200 p-2 text-rose-700 hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-45" title="Xóa bài làm" aria-label={`Xóa bài làm của ${item.respondent_name}`}><Trash2 className="h-4 w-4" /></button></div></td></tr>) : <tr><td colSpan={11} className="py-10 text-center text-slate-500">Chưa có lượt làm bài.</td></tr>}</tbody></table></div>
        </div>
      </section>
    );
  }
