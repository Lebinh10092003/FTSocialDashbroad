...
export default function TrainingAssessmentsAdmin({
    ...
    try {
      ...
      if (sourceMode === "xlsx") {
        ...
        response = await fetch("/api/digital-training/assessments/import-preview", {
          ...
          body: JSON.stringify({ google_sheet_url: sheetUrl.trim() }),
        });
      }
      if (!response.ok) throw new Error(await errorText(response));
  ...
  const downloadTemplate = () => {
    const rows = ["Đề 1", "Đề 2", "Đề 3", "Đề 4", "Đề 5"].map((variant, index) => ({
      "Mã đề": variant,
      STT: 1,
      "Loại câu": index === 4 ? "Tải ảnh" : "Trắc nghiệm",
      "Câu hỏi": index === 4 ? "Tải ảnh kết quả bài thực hành." : "Nội dung câu hỏi mẫu",
      A: index === 4 ? "" : "Phương án A",
      B: index === 4 ? "" : "Phương án B",
      C: index === 4 ? "" : "Phương án C",
      D: index === 4 ? "" : "Phương án D",
      "Đáp án": index === 4 ? "" : "B",
      Điểm: 1,
      "Bắt buộc": "Có",
      "Giải thích": "",
    ...
    XLSX.utils.book_append_sheet(workbook, XLSX.utils.json_to_sheet(rows), "Câu hỏi");
    XLSX.writeFile(workbook, "mau_bai_kiem_tra_tap_huan.xlsx");
  };
      ...
      <section className="mt-6 space-y-5">
        ...
        <div className="rounded-2xl border bg-white p-6 shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div><p className="text-xs font-bold uppercase tracking-wider text-blue-600">Thiết lập bài cuối học phần</p><h2 className="mt-1 text-2xl font-extrabold">Tạo một link, chia đều nhiều mã đề</h2><p className="mt-2 text-sm text-slate-500">Nhập câu hỏi từ XLSX hoặc Google Sheet, kiểm tra dữ liệu rồi phát hành.</p></div>
            <button onClick={downloadTemplate} className="ft-btn ft-btn-secondary"><Download className="h-4 w-4" />Tải file mẫu</button>
          </div>
          <div className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,.8fr)]">
            <div className="space-y-5">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="sm:col-span-2"><span className="mb-1 block text-sm font-bold">Đơn vị / phân lớp *</span><select className="ft-input" value={draft.target} onChange={(event) => setDraft({ ...draft, target: event.target.value })}><option value="">Chọn đơn vị hoặc phân lớp</option>{targets.map((target) => <option key={target.value} value={target.value}>{target.label}</option>)}</select><small className="mt-1 block text-slate-500">Mỗi đơn vị/phân lớp chỉ có một bài cuối học phần và một link công khai.</small></label>
            ...
            <div className="rounded-2xl border bg-slate-50 p-5">
              <h3 className="font-extrabold">Nguồn câu hỏi</h3>
              ...
              {sourceMode === "xlsx" ? <label className="mt-4 flex cursor-pointer flex-col items-center rounded-xl border-2 border-dashed border-slate-300 bg-white p-6 text-center"><FileSpreadsheet className="h-9 w-9 text-emerald-600" /><b className="mt-2">{file?.name || "Chọn file .xlsx"}</b><span className="mt-1 text-xs text-slate-500">Tối đa 10 MB</span><input type="file" accept=".xlsx" className="hidden" onChange={(event) => { setFile(event.target.files?.[0] || null); setPreview(null); }} /></label> : <label className="mt-4 block"><span className="mb-1 block text-sm font-bold">Đường dẫn Google Sheet</span><input className="ft-input" value={sheetUrl} onChange={(event) => { setSheetUrl(event.target.value); setPreview(null); }} placeholder="https://docs.google.com/spreadsheets/d/..." /><small className="mt-2 block text-slate-500">Sheet cần bật “Bất kỳ ai có đường liên kết đều có thể xem”.</small></label>}
              <button disabled={busy} onClick={importQuestions} className="ft-primary mt-4 w-full justify-center"><Upload className="h-4 w-4" />Đọc và kiểm tra dữ liệu</button>
              {preview && <div className="mt-4 space-y-3">
                <div className="grid grid-cols-2 gap-2"><div className="rounded-lg bg-white p-3"><b className="text-xl">{preview.variants.length}</b><span className="block text-xs text-slate-500">mã đề</span></div><div className="rounded-lg bg-white p-3"><b className="text-xl">{preview.question_count}</b><span className="block text-xs text-slate-500">dòng câu hỏi</span></div></div>
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t pt-5">
            ...
            <button disabled={busy || !preview || preview.errors.length > 0} onClick={createAssessment} className="ft-primary disabled:opacity-50">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Tạo bài cuối học phần</button>
          </div>
        </div>
      </section>
    );
      ...
      <section className="mt-6 space-y-5">
        ...
        <article className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4 p-6">
            <div><p className="text-xs font-bold uppercase text-blue-600">Bài đánh giá cuối học phần</p><h2 className="mt-1 text-2xl font-extrabold">{selected.title}</h2><p className="mt-2 text-sm text-slate-500">{[selected.partner_name, selected.class_name].filter(Boolean).join(" · ")}</p></div>
          ...
          <div className="grid gap-5 border-t bg-slate-50 p-6 lg:grid-cols-[minmax(0,1fr)_220px]">
            <div>
              <div className="flex items-center gap-2"><span className={`rounded-full px-3 py-1 text-xs font-bold ${selected.status === "published" ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{statusLabel[selected.status]}</span><span className="text-sm text-slate-500">{selected.duration_minutes} phút · tối đa {selected.attempt_limit} lượt/người</span></div>
