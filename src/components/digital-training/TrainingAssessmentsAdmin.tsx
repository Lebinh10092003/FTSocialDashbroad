        ...
        <article className="assessment-detail-card overflow-hidden rounded-2xl border bg-white shadow-sm">
          ...
          <div className={`${detailTab === "overview" ? "grid" : "hidden"} gap-5 border-t bg-slate-50 p-6 lg:grid-cols-[minmax(0,1fr)_220px]`}>
            <div>
              ...
              <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm font-extrabold text-slate-900">Xem trước bài kiểm tra</p><p className="mt-1 text-xs text-slate-500">Hai chế độ chỉ dùng để kiểm tra giao diện, nội dung và đáp án; không tạo lượt làm hoặc làm thay đổi số liệu.</p></div><div className="flex flex-wrap gap-2"><a href={`${publicLink}?preview=creator`} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary" title="Xem đáp án, nội dung và cấu hình theo từng mã đề"><Layers3 className="h-4 w-4" />Xem trước · Chế độ quản trị viên</a><a href={`${publicLink}?preview=respondent`} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary" title="Mô phỏng giao diện người làm bài; không tạo lượt làm"><ExternalLink className="h-4 w-4" />Xem trước · Chế độ học viên</a></div></div></section>
            </div>
