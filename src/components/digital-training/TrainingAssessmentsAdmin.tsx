...
export default function TrainingAssessmentsAdmin({
  ...
  const createAssessment = async () => {
    ...
    if (!participantRows.length) {
      setNotice("Vui lòng nhập danh sách người tham gia trước khi tạo đợt kiểm tra.");
      return;
    }
            ...
            <div className="rounded-2xl border bg-slate-50 p-5">
              ...
              {preview && <div className="mt-4 space-y-3">
                ...
                {preview.import_mode === "auto_generate" && !!structureRows.length && <div className="rounded-xl border bg-white p-3">
                  <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase text-slate-500">Cơ cấu chủ đề / độ khó</p><p className="mt-1 text-xs text-slate-500">Để trống nếu muốn hệ thống chia theo tỷ lệ ngân hàng.</p></div><b className={`text-sm ${structureTotal > 0 && structureTotal !== Number(questionsPerVariant) ? "text-rose-600" : "text-emerald-700"}`}>{structureTotal || "Tự động"}{structureTotal > 0 ? ` / ${questionsPerVariant} cau` : ""}</b></div>
                  <div className="mt-3 max-h-64 space-y-2 overflow-y-auto">{structureRows.map((row) => <label key={row.key} className="grid grid-cols-[minmax(0,1fr)_72px] items-center gap-2 rounded-lg bg-slate-50 p-2"><span className="text-xs"><b>{row.category || "Không chủ đề"}</b><span className="block text-[11px] text-slate-500">{row.difficulty || "Không độ khó"} · có {row.available} cau</span></span><input type="number" min="0" max={row.available} className="rounded-lg border px-2 py-1.5 text-sm" value={structureRules[row.key] || ""} placeholder="0" onChange={(event) => { setStructureRules((current) => ({ ...current, [row.key]: event.target.value })); setStructureDirty(true); }} /></label>)}</div>
