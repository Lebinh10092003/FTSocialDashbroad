      ...
      <section className="mt-6 space-y-5">
        ...
        <article className="overflow-hidden rounded-2xl border bg-white shadow-sm">
          <div className="flex flex-wrap items-start justify-between gap-4 p-6">
            ...
            <div className="flex flex-wrap gap-2">{selected.status !== "published" && <button disabled={busy} onClick={() => void changeStatus("published")} className="ft-primary"><Send className="h-4 w-4" />{selected.status === "closed" ? "Mở lại bài" : "Phát hành"}</button>}{selected.status === "published" && <button disabled={busy} onClick={() => void changeStatus("closed")} className="ft-btn ft-btn-secondary">Đóng bài</button>}{selected.output_sheet_url && <a href={selected.output_sheet_url} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary" title="Mở Google Sheet đầu ra"><FileSpreadsheet className="h-4 w-4" />Mở Sheet đầu ra<ExternalLink className="h-3.5 w-3.5" /></a>}{driveFolderLink && <a href={driveFolderLink} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary" title="Mở thư mục Google Drive lưu bài làm"><Upload className="h-4 w-4" />Mở thư mục bài làm<ExternalLink className="h-3.5 w-3.5" /></a>}<button disabled={busy || !selected.output_sheet_url || selected.sync_status === "ready"} onClick={() => void prepareOutput()} className="ft-btn ft-btn-secondary" title={selected.sync_status === "ready" ? "Sheet đầu ra đã được khởi tạo" : "Khởi tạo các tab Sheet đầu ra"}><FileSpreadsheet className="h-4 w-4" />{selected.sync_status === "ready" ? "Sheet đầu ra đã sẵn sàng" : "Khởi tạo Sheet đầu ra"}</button><button aria-label="Xóa bài đánh giá" title="Xóa bài đánh giá" onClick={() => void remove()} className="rounded-lg border border-rose-200 px-3 py-2 text-sm font-bold text-rose-700"><Trash2 className="h-4 w-4" /></button></div>
          </div>
          ...
          {detailTab === "settings" && (selected.output_sheet_url || driveFolderLink) && <div className="border-t bg-slate-50 px-6 pb-6"><div className="mx-auto max-w-4xl rounded-xl border border-blue-100 bg-blue-50/60 p-4"><p className="text-sm font-extrabold text-slate-900">Dữ liệu đầu ra</p><p className="mt-1 text-sm text-slate-600">Theo dõi trực tiếp Sheet tổng hợp và thư mục lưu bài làm của người học.</p><div className="mt-3 flex flex-wrap gap-2">{selected.output_sheet_url && <a href={selected.output_sheet_url} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary bg-white"><FileSpreadsheet className="h-4 w-4" />Mở Sheet đầu ra<ExternalLink className="h-3.5 w-3.5" /></a>}{driveFolderLink && <a href={driveFolderLink} target="_blank" rel="noreferrer" className="ft-btn ft-btn-secondary bg-white"><Upload className="h-4 w-4" />Mở thư mục bài làm<ExternalLink className="h-3.5 w-3.5" /></a>}</div></div></div>}
        </article>
