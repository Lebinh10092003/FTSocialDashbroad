...
export default function ProductManagement({
  ...
  const [view, setView] = useState<"matrix" | "statistics">("matrix");
  ...
  const [productDraft, setProductDraft] = useState({ name: "", description: "" });
  ...
  const productList = products.filter((item) => item.active).sort((a, b) => a.display_order - b.display_order || a.name.localeCompare(b.name, "vi"));
  ...
  const filteredPartners = useMemo(() => partners.filter((partner) => {
    ...
    return (!filters.query || searchable.includes(filters.query.toLocaleLowerCase("vi-VN")))
  ...
  const saveProduct = async (event: React.FormEvent) => {
    ...
    setNotice("");
    try {
      const response = await fetch("/api/digital-training/products", {
        method: "POST",
        headers: { ...auth, "Content-Type": "application/json" },
    ...
    <section className="mt-1 space-y-5">
      <header className="rounded-2xl border bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div><p className="text-xs font-bold uppercase tracking-wide text-blue-600">Danh mục và mức sử dụng</p><h1 className="mt-1 text-2xl font-extrabold">Quản lý sản phẩm</h1><p className="mt-2 text-sm text-slate-500">Theo dõi số lượng, hạn sử dụng và trạng thái sản phẩm của từng khách hàng.</p></div>
          <div className="flex flex-wrap gap-2"><button onClick={() => void load()} className="ft-btn ft-btn-secondary"><RefreshCw className={`h-4 w-4 ${busy ? "animate-spin" : ""}`} />Làm mới</button>{!isGuest && <button onClick={() => setProductOpen(true)} className="ft-primary"><PackagePlus className="h-4 w-4" />Thêm sản phẩm</button>}</div>
        </div>
        <div className="mt-5 flex gap-2 border-t pt-4"><button onClick={() => setView("matrix")} className={`rounded-lg px-4 py-2 text-sm font-bold ${view === "matrix" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}>Bảng khách hàng</button><button onClick={() => setView("statistics")} className={`rounded-lg px-4 py-2 text-sm font-bold ${view === "statistics" ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-600"}`}>Thống kê</button></div>
      </header>

      <div className="grid gap-3 rounded-2xl border bg-white p-4 shadow-sm md:grid-cols-2 xl:grid-cols-7">
      ...
      {view === "matrix" ? <article className="overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3 p-4"><div><h2 className="font-extrabold">Ma trận sản phẩm theo khách hàng</h2><p className="text-xs text-slate-500">{filteredPartners.length} khách hàng · {activeSubscriptions.length} đăng ký đang sử dụng · tổng số lượng {totalQuantity}</p></div></div>
        <div className="overflow-x-auto"><table className="ft-table min-w-[1200px]"><thead><tr><th>STT</th><th className="min-w-56">Tên khách hàng</th><th>Nhóm / loại</th>{productList.map((product) => <th key={product.id} className="min-w-48">{product.name}<span className="block text-[10px] font-normal text-slate-400">{product.description}</span></th>)}<th className="min-w-64">Ghi chú</th></tr></thead><tbody>{filteredPartners.length ? filteredPartners.map((partner, index) => <tr key={partner.id}><td>{index + 1}</td><td><b>{partner.name}</b><span className="block text-xs text-slate-500">{[partner.ward, partner.province].filter(Boolean).join(" · ") || "Chưa có địa bàn"}</span></td><td><b className="text-xs">{partner.partner_type || "—"}</b><span className="block text-xs text-slate-500">{partner.partner_subtype || "—"}</span></td>{productList.map((product) => { const item = subscriptionsByCell.get(`${partner.id}:${product.id}`); return <td key={product.id}>{item ? <button type="button" onClick={() => !isGuest && openSubscription(partner.id, product.id, item)} className="w-full rounded-xl border border-slate-200 p-2 text-left hover:border-blue-300 hover:bg-blue-50"><span className="flex items-center justify-between gap-2"><b className="text-sm">SL: {item.quantity}</b>{!isGuest && <Pencil className="h-3.5 w-3.5 text-blue-600" />}</span><span className={`mt-1 inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold ${statusClass[item.effective_status]}`}>{statusNames[item.effective_status]}</span><span className="mt-1 block text-[11px] text-slate-500">Hạn: {showDate(item.expires_at)}</span>{item.days_remaining != null && <span className={`block text-[11px] font-semibold ${item.days_remaining < 0 ? "text-rose-600" : item.days_remaining <= 30 ? "text-amber-600" : "text-emerald-600"}`}>{item.days_remaining < 0 ? `Quá hạn ${Math.abs(item.days_remaining)} ngày` : `Còn ${item.days_remaining} ngày`}</span>}</button> : !isGuest ? <button type="button" onClick={() => openSubscription(partner.id, product.id)} className="w-full rounded-lg border border-dashed px-2 py-3 text-xs font-bold text-slate-400 hover:border-blue-300 hover:text-blue-600"><Plus className="mr-1 inline h-3.5 w-3.5" />Đăng ký</button> : <span className="text-xs text-slate-400">Không đăng ký</span>}</td>; })}<td className="max-w-sm whitespace-normal text-xs"><b className="block text-slate-700">{partner.notes || "—"}</b>{subscriptions.filter((item) => item.partner === partner.id && item.notes).map((item) => <span key={item.id} className="mt-1 block text-slate-500">{item.product_name}: {item.notes}</span>)}</td></tr>) : <tr><td colSpan={4 + productList.length} className="py-12 text-center text-slate-500">Không có khách hàng phù hợp bộ lọc.</td></tr>}</tbody></table></div>
      ...
      {productOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4"><form onSubmit={saveProduct} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><h2 className="text-xl font-extrabold">Thêm sản phẩm mới</h2><p className="mt-1 text-sm text-slate-500">Sản phẩm mới sẽ tự tạo thêm một cột trong bảng quản lý.</p></div><button type="button" onClick={() => setProductOpen(false)}><X className="h-5 w-5" /></button></div><label className="mt-5 block"><span className="mb-1 block text-sm font-bold">Tên sản phẩm *</span><input required className="ft-input" value={productDraft.name} onChange={(event) => setProductDraft({ ...productDraft, name: event.target.value })} /></label><label className="mt-4 block"><span className="mb-1 block text-sm font-bold">Mô tả</span><textarea className="ft-input min-h-24" value={productDraft.description} onChange={(event) => setProductDraft({ ...productDraft, description: event.target.value })} /></label>{notice && <p className="mt-4 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">{notice}</p>}<div className="mt-6 flex justify-end gap-2"><button type="button" onClick={() => setProductOpen(false)} className="ft-btn ft-btn-secondary">Hủy</button><button disabled={busy} className="ft-primary">Thêm sản phẩm</button></div></form></div>}
    </section>
  );
}
