...
import { isDarkEmailColor, resolveEmailContainerTextColor, resolveEmailLineHeight } from '../../lib/emailPresentation';
        ...
        {block.type === 'columns' && layoutState && <div className="space-y-2">
          ...
          {(() => {
            ...
            const renderLayoutCell = (columnIndex: number, cellIndex: number, rowHeight = 0) => {
              ...
              const cellTextColor = resolveEmailContainerTextColor(cell.background, cell.color, inheritedTextColor, emailSettings.textColor || '#1e293b');
              return <div key={cell.id} onDragOver={event => { event.preventDefault(); event.stopPropagation(); setRootDragOver(false); event.dataTransfer.dropEffect = event.dataTransfer.types.includes('application/x-ft-email-block-id') ? 'move' : 'copy'; }} onDrop={event => dropInto(event, block, slotIndex)} className="group/cell relative flex min-w-0 flex-col transition hover:outline hover:outline-2 hover:outline-blue-300" style={{ minHeight: minimumHeight || undefined, justifyContent: cell.verticalAlign === 'middle' ? 'center' : cell.verticalAlign === 'bottom' ? 'flex-end' : 'flex-start', backgroundColor: cell.background, color: cellTextColor, padding: cell.padding, border: cell.borderWidth ? `${cell.borderWidth}px solid ${cell.borderColor}` : 'none', borderRadius: cell.borderRadius, overflow: 'visible' }}>
                <div className="flex min-w-0 flex-1 flex-col">{slot.map((child, childIndex, siblings) => renderBlock(child, childIndex, siblings, shouldFill, cellTextColor))}</div>
