...
const EmailCanvas = React.forwardRef<EmailCanvasHandle, EmailCanvasProps>(function EmailCanvas(props, ref) {
  ...
  const renderSimplePreview = (block: EmailBlock) => {
    ...
    if (block.type === 'custom-html') return <div className="space-y-2">{selected && <textarea value={content.html || ''} onChange={event => update({ html: event.target.value })} onClick={event => event.stopPropagation()} rows={6} spellCheck={false} className="w-full rounded-lg border border-blue-200 bg-slate-950 p-3 font-mono text-[11px] text-slate-100 outline-none focus:border-blue-500" />}<iframe title="Xem trước HTML" sandbox="" srcDoc={`<!doctype html><html><body style="margin:0">${sanitizeCustomHtml(content.html || '')}</body></html>`} className="min-h-24 w-full rounded border bg-white" /></div>;
