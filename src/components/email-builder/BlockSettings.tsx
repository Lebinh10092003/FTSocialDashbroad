...
export default function BlockSettings({ block, onUpdateBlockContent, onUpdateBlockStyles, onUpdateBlockColumns, onUpdateBlock, onApplySelectionFontSize, onApplySelectionTextColor }: BlockSettingsProps) {
  ...
  return <div className="h-full space-y-5 overflow-y-auto bg-white p-5 text-slate-800">
    ...
    {block.type === 'heading' && <div className="space-y-4"><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Nội dung tiêu đề</label><textarea value={content.text || ''} onChange={event => updateContent('text', event.target.value)} rows={3} className={fieldClass} /></div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Cấp tiêu đề</label><select value={content.level || 'h2'} onChange={event => updateContent('level', event.target.value)} className={fieldClass}><option value="h1">H1 — Chính</option><option value="h2">H2 — Phụ</option><option value="h3">H3 — Mục nhỏ</option></select></div><NumberDraft label="Cỡ chữ (px)" value={content.fontSize ?? 20} min={10} max={72} onCommit={value => updateContent('fontSize', value === '' ? 20 : value)} /></div><ColorField label="Màu chữ" value={content.color || '#0F3A72'} onChange={value => updateContent('color', value)} />{alignControl}</div>}
    ...
    {block.type === 'button' && <section className="space-y-3">
      ...
      <div><label className="mb-1 block text-[10px] font-bold text-slate-500">Nhãn nút</label><textarea value={content.text || ''} onChange={event => updateContent('text', event.target.value)} rows={2} className={fieldClass} /></div>
    ...
    {isButtonGroup && <section className="space-y-3">
      ...
      {groupButtons().map((button: any, index: number) => <div key={index} className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
        ...
        <textarea value={button.text || ''} onChange={event => updateGroupButton(index, { text: event.target.value })} rows={2} placeholder="Nhãn nút" className={fieldClass} />
