...
export default function BlockSettings({ block, variables = [], onUpdateBlockContent, onUpdateBlockStyles, onUpdateBlockColumns, onUpdateBlock, onSplitCustomHtml, onApplySelectionFontSize, onApplySelectionTextColor, hasTextSelection = false, selectionFontSize, selectionTextColor, selectionEditorKey }: BlockSettingsProps) {
  ...
  return <div className="h-full space-y-5 overflow-y-auto bg-white p-5 text-slate-800">
    ...
    {block.type === 'icon-text' && <div className="space-y-4">
      ...
      {uploadError && <p className="rounded-lg bg-rose-50 p-2 text-[10px] font-bold text-rose-600">{uploadError}</p>}
      <div className="grid grid-cols-2 gap-3"><NumberDraft label="Kích thước icon (px)" value={content.iconSize ?? 24} min={8} max={160} onCommit={value => updateContent('iconSize', value === '' ? 24 : value)} /><NumberDraft label="Khoảng cách icon–chữ (px)" value={content.gap ?? 10} max={80} onCommit={value => updateContent('gap', value === '' ? 0 : value)} /><NumberDraft live label="Cỡ chữ (px)" value={content.fontSize ?? 15} min={8} max={96} onCommit={value => updateContent('fontSize', value === '' ? 15 : value)} /></div>
      <div className="grid grid-cols-2 gap-2">{content.iconSource !== 'upload' && <ColorField label="Màu icon" value={content.iconColor || '#1473D1'} onChange={value => onUpdateBlockContent({ ...content, iconColor: value, iconPngUrl: '', iconRasterKey: '' })} />}<ColorField label="Màu chữ" value={content.color || '#1E293B'} onChange={value => updateContent('color', value)} /></div>
      {alignControl}
    ...
    {block.type === 'heading' && <div className="space-y-4"><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Nội dung tiêu đề</label><textarea value={content.text || ''} onChange={event => onUpdateBlockContent({ ...content, text: event.target.value, html: '' })} rows={3} className={fieldClass} /></div><div className="grid grid-cols-2 gap-3"><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Cấp tiêu đề</label><select value={content.level || 'h2'} onChange={event => updateContent('level', event.target.value)} className={fieldClass}><option value="h1">H1 — Chính</option><option value="h2">H2 — Phụ</option><option value="h3">H3 — Mục nhỏ</option></select></div><NumberDraft live label="Cỡ chữ (px)" value={hasTextSelection ? selectionFontSize : content.fontSize ?? 20} min={8} max={96} onCommit={value => { const size = value === '' ? 20 : value; if (!onApplySelectionFontSize?.(size)) updateContent('fontSize', size); }} /></div><ColorField label={hasTextSelection ? 'Màu chữ vùng chọn' : 'Màu chữ'} value={hasTextSelection && selectionTextColor ? selectionTextColor : content.color || '#0F3A72'} onChange={value => { if (!onApplySelectionTextColor?.(value)) updateContent('color', value); }} />{alignControl}</div>}

    {block.type === 'signature-builder' && <section className="space-y-4 rounded-xl border border-orange-100 bg-orange-50/40 p-3.5">
      <div><h4 className="text-[10px] font-black uppercase text-orange-800">Trình tạo chữ ký</h4><p className="mt-1 text-[9px] leading-relaxed text-slate-500">Xuất chữ ký bảng tương thích Gmail/Outlook. Các kênh bật sẽ hiển thị dưới thông tin liên hệ.</p></div>
      <div className="grid grid-cols-2 gap-2"><div className="col-span-2"><label className="mb-1 block text-[10px] font-bold text-slate-500">URL logo</label><input value={content.logoUrl || ''} onChange={event => updateContent('logoUrl', event.target.value)} placeholder="https://..." className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Họ và tên</label><input value={content.fullName || ''} onChange={event => updateContent('fullName', event.target.value)} className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Chức vụ</label><input value={content.jobTitle || ''} onChange={event => updateContent('jobTitle', event.target.value)} className={fieldClass} /></div><div className="col-span-2"><label className="mb-1 block text-[10px] font-bold text-slate-500">Công ty</label><input value={content.company || ''} onChange={event => updateContent('company', event.target.value)} className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Điện thoại</label><input value={content.phone || ''} onChange={event => updateContent('phone', event.target.value)} className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Email</label><input value={content.email || ''} onChange={event => updateContent('email', event.target.value)} className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Website</label><input value={content.website || ''} onChange={event => updateContent('website', event.target.value)} className={fieldClass} /></div><div><label className="mb-1 block text-[10px] font-bold text-slate-500">Địa chỉ</label><input value={content.address || ''} onChange={event => updateContent('address', event.target.value)} className={fieldClass} /></div></div>
      <div className="grid grid-cols-2 gap-3"><NumberDraft label="Rộng chữ ký (px)" value={content.width ?? 650} min={320} max={900} onCommit={value => updateContent('width', value === '' ? 650 : value)} /><NumberDraft label="Rộng logo (px)" value={content.logoWidth ?? 150} min={48} max={320} onCommit={value => updateContent('logoWidth', value === '' ? 150 : value)} /><NumberDraft label="Cỡ tên (px)" value={content.titleSize ?? 22} min={12} max={48} onCommit={value => updateContent('titleSize', value === '' ? 22 : value)} /><NumberDraft label="Cỡ thông tin (px)" value={content.bodySize ?? 12} min={9} max={24} onCommit={value => updateContent('bodySize', value === '' ? 12 : value)} /><NumberDraft label="Đệm trong (px)" value={content.padding ?? 18} min={0} max={48} onCommit={value => updateContent('padding', value === '' ? 18 : value)} /></div>
      <div className="grid grid-cols-2 gap-2"><ColorField label="Màu nhấn" value={content.accentColor || '#1473D1'} onChange={value => updateContent('accentColor', value)} /><ColorField label="Màu chữ" value={content.textColor || '#28323D'} onChange={value => updateContent('textColor', value)} /></div>
      <div className="space-y-2"><p className="text-[10px] font-black uppercase text-slate-600">Mạng xã hội & liên kết</p>{[['Facebook','facebook'],['LinkedIn','linkedIn'],['YouTube','youtube'],['Instagram','instagram'],[content.otherLabel || 'Kênh khác','other']].map(([label, key]) => { const titleKey = `${key.charAt(0).toUpperCase()}${key.slice(1)}`; const visibleKey = `show${titleKey}`; const urlKey = `${key}Url`; return <div key={key} className="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-2"><label className="flex items-center gap-1.5 text-[10px] font-bold text-slate-700"><input type="checkbox" checked={content[visibleKey] !== false} onChange={event => updateContent(visibleKey, event.target.checked)} />{label}</label><input value={content[urlKey] || ''} onChange={event => updateContent(urlKey, event.target.value)} placeholder="https://..." className={fieldClass} /></div>; })}</div>
      <div><label className="mb-1 block text-[10px] font-bold text-slate-500">Nhãn kênh khác</label><input value={content.otherLabel || ''} onChange={event => updateContent('otherLabel', event.target.value)} className={fieldClass} /></div>
    </section>}
    {['paragraph', 'signature', 'highlight-box'].includes(block.type) && <section className="space-y-3 rounded-xl border border-blue-100 bg-blue-50/40 p-3.5"><h4 className="text-[10px] font-black uppercase">Định dạng mặc định email-safe</h4><div className="grid grid-cols-2 gap-3"><NumberDraft live label="Cỡ chữ (px)" value={hasTextSelection ? selectionFontSize : content.fontSize ?? 15} min={8} max={96} onCommit={value => { const size = value === '' ? 15 : value; if (!onApplySelectionFontSize?.(size)) updateContent('fontSize', size); }} /><NumberDraft label="Giãn dòng" value={content.lineHeight ?? 1.6} min={1} max={2.4} onCommit={value => updateContent('lineHeight', value === '' ? 1.6 : value)} /></div><ColorField label={hasTextSelection ? 'Màu chữ vùng chọn' : 'Màu chữ mặc định'} value={hasTextSelection && selectionTextColor ? selectionTextColor : content.color || '#1E293B'} onChange={value => { if (!onApplySelectionTextColor?.(value)) updateContent('color', value); }} />{alignControl}<p className="text-[9px] text-slate-500">Bôi đen văn bản trên canvas để đổi riêng cỡ chữ hoặc màu chữ cho vùng đã chọn.</p></section>}
    ...
    {block.type === 'columns' && layoutState && <section className="space-y-4 rounded-xl border border-blue-100 bg-blue-50/40 p-3.5">
      ...
      {layoutState.layout.map((column, columnIndex) => <div key={column.id} className="space-y-3 rounded-xl border border-slate-200 bg-white p-3">
        <div className="flex items-end justify-between gap-2"><div className="min-w-0 flex-1"><NumberDraft live label={`Tỷ lệ rộng cột ${columnIndex + 1}`} value={column.width} min={0.25} max={12} step={0.25} onCommit={value => commitLayout(updateEmailLayoutColumn(block, columnIndex, { width: value === '' ? 1 : value }))} /><p className="mt-1 text-[8px] font-bold text-blue-600">≈ {(column.width / layoutState.layout.reduce((sum, item) => sum + item.width, 0) * 100).toFixed(1)}% trước khi áp dụng giới hạn</p></div><button type="button" disabled={column.cells.length >= 4} onClick={() => commitLayout(addEmailLayoutCell(block, columnIndex))} className="mb-0.5 inline-flex h-9 items-center gap-1 rounded-lg border border-blue-200 px-2 text-[9px] font-bold text-blue-700 disabled:opacity-40"><Plus className="h-3.5 w-3.5" />Thêm ô</button></div>
        <div className="grid grid-cols-2 gap-2"><NumberDraft label="Rộng tối thiểu (px)" value={column.minWidth || ''} min={0} max={1200} onCommit={value => commitLayout(updateEmailLayoutColumn(block, columnIndex, { minWidth: value === '' ? 0 : value }))} /><NumberDraft label="Rộng tối đa (px)" value={column.maxWidth || ''} min={0} max={1200} onCommit={value => commitLayout(updateEmailLayoutColumn(block, columnIndex, { maxWidth: value === '' ? 0 : value }))} /></div>
        {column.cells.map((cell, cellIndex) => <div key={cell.id} className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
          ...
          <div><label className="mb-1 block text-[10px] font-bold text-slate-500">Chiều cao</label><select value={cell.heightMode} onChange={event => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { heightMode: event.target.value as 'auto' | 'fixed' }))} className={fieldClass}><option value="auto">Tự động — bằng ô cao nhất cùng hàng</option><option value="fixed">Đặt chiều cao hàng</option></select></div>
          <div className="grid grid-cols-2 gap-2"><NumberDraft label="Đệm trong (px)" value={cell.padding} max={64} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { padding: value === '' ? 0 : value }))} /><NumberDraft label="Cao tối thiểu (px)" value={cell.minHeight || ''} min={0} max={1200} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { minHeight: value === '' ? 0 : value }))} />{cell.heightMode === 'fixed' && <NumberDraft live label="Chiều cao hàng (px)" value={cell.height} min={24} max={1200} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { height: value === '' ? 96 : value }))} />}<NumberDraft label="Cao tối đa (px)" value={cell.maxHeight || ''} min={0} max={1600} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { maxHeight: value === '' ? 0 : value }))} /><NumberDraft label="Độ dày viền (px)" value={cell.borderWidth} max={12} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { borderWidth: value === '' ? 0 : value }))} /><NumberDraft label="Bo góc (px)" value={cell.borderRadius} max={80} onCommit={value => commitLayout(updateEmailLayoutCell(block, columnIndex, cellIndex, { borderRadius: value === '' ? 0 : value }))} /></div>
          <p className="text-[8px] leading-relaxed text-slate-400">Để trống/0 nghĩa là không giới hạn. Giới hạn cao áp dụng cho kích thước đặt trước; nội dung dài vẫn được nở ra để không bị cắt trong email.</p>
    ...
    {block.type === 'button' && <section className="space-y-3">
      ...
      {alignControl}
      <div className="grid grid-cols-2 gap-2"><ColorField label="Màu nền" value={content.bg || '#1473d1'} onChange={value => updateContent('bg', value)} /><ColorField label={hasTextSelection ? 'Màu chữ vùng chọn' : 'Màu chữ'} value={hasTextSelection && selectionTextColor ? selectionTextColor : content.color || '#ffffff'} onChange={value => { if (!onApplySelectionTextColor?.(value)) updateContent('color', value); }} /></div>
    </section>}
