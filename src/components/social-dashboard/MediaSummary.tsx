...
interface TrendPoint { period: string; label: string; views: number; engagement: number; postsCount: number; followers: number; startValues?: Record<string, number>; endValues?: Record<string, number>; }
  ...
  return <div className="space-y-7 pb-10">
    ...
    {loading ? <div className="py-24 text-center bg-white border border-slate-200 rounded-3xl"><RefreshCw className="w-9 h-9 text-blue-500 animate-spin mx-auto"/><p className="text-sm text-slate-500 mt-3">Đang tải dữ liệu báo cáo...</p></div> : error ? <div className="p-6 rounded-2xl bg-rose-50 border border-rose-200 text-rose-800 flex items-center gap-3"><AlertCircle className="w-6 h-6"/>{error}</div> : <>
      ...
      <section className="bg-white rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100"><h3 className="text-lg font-extrabold text-slate-800">Giá trị trong kỳ và biến động</h3><p className="text-sm text-slate-500 mt-1">{activeMetricConfig.label}: giá trị trong kỳ = mốc cuối kỳ trừ mốc đầu kỳ. Với kỳ hiện tại, mốc cuối là chỉ số đang có.</p></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left"><thead><tr className="bg-slate-50 text-xs font-bold uppercase tracking-wide text-slate-500"><th className="px-5 py-4">Kỳ báo cáo</th><th className="px-5 py-4 text-right">Mốc đầu kỳ</th><th className="px-5 py-4 text-right">Mốc cuối kỳ</th><th className="px-5 py-4 text-right">Giá trị trong kỳ</th><th className="px-5 py-4 text-right">So kỳ trước</th></tr></thead><tbody className="divide-y divide-slate-100 text-sm">{deltaRows.map(({point,index}) => { const change = percentChange(point.value, chartData[index - 1]?.value); const start = point.startValues?.[activeMetric] ?? 0; const end = point.endValues?.[activeMetric] ?? point.value; return <tr key={point.period} className="hover:bg-slate-50/70"><td className="px-5 py-4 font-bold text-slate-800">{point.label}</td><td className="px-5 py-4 text-right text-slate-600">{Number(start).toLocaleString('vi-VN')}</td><td className="px-5 py-4 text-right text-slate-600">{Number(end).toLocaleString('vi-VN')}</td><td className="px-5 py-4 text-right font-extrabold text-slate-800">{Number(point.value).toLocaleString('vi-VN')}</td><td className="px-5 py-4 text-right"><span className={`inline-flex items-center gap-1 font-bold ${change === null ? 'text-slate-400' : change >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>{change === null ? '—' : <>{change >= 0 ? <TrendingUp className="w-3.5 h-3.5"/> : <TrendingDown className="w-3.5 h-3.5"/>}{Math.abs(change).toFixed(1)}%</>}</span></td></tr>})}</tbody></table></div>
