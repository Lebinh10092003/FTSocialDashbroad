...
`;
      ...
      case 'signature-builder': {
        ...
        const normalizedSocialUrl = (urlKey: string) => urlKey === 'zaloUrl' && /^\+?[\d\s.-]+$/.test(String(content[urlKey] || '').trim())
          ? `https://zalo.me/${String(content[urlKey]).replace(/\D/g, '')}` : content[urlKey];
        const socialLinks = socialDefinitions.filter(([, , urlKey, visibleKey]) => content[visibleKey] !== false && content[urlKey]);
        socialLinks.forEach(([, , urlKey]) => checkLinkUrl(normalizedSocialUrl(urlKey), 'Liên kết chữ ký'));
        const detailDefinitions = [
          ['Đ', 'Địa chỉ', 'address', 'showAddress'], ['M', 'Di động', 'phone', 'showPhone'],
          ['E', 'Email', 'email', 'showEmail'], ['E', 'Email phụ', 'secondaryEmail', 'showSecondaryEmail'],
          ['W', 'Website', 'website', 'showWebsite'],
        ];
        const details = detailDefinitions
          .filter(([, , valueKey, visibleKey]) => content[visibleKey] !== false && content[valueKey])
          .map(([mark, label, valueKey]) => `<div style="padding:2px 0;"><span style="display:inline-block;width:18px;height:18px;line-height:18px;margin-right:6px;border-radius:9px;background:${accent};color:#ffffff;text-align:center;font-size:9px;font-weight:bold;">${mark}</span><strong>${label}:</strong> ${rep(content[valueKey])}</div>`);
        const customFields = (Array.isArray(content.customFields) ? content.customFields : []).filter((field: any) => field && field.visible !== false && (field.label || field.value));
        customFields.forEach((field: any) => { if (field.url) checkLinkUrl(field.url, 'Trường tùy ý chữ ký'); });
        const customDetails = customFields.map((field: any) => {
          const value = field.url ? `<a href="${rep(field.url)}" target="_blank" rel="noopener noreferrer" style="color:${accent};text-decoration:none;">${rep(field.value || field.url)}</a>` : rep(field.value || '');
          return `<div style="padding:2px 0;"><span style="display:inline-block;width:18px;height:18px;line-height:18px;margin-right:6px;border-radius:9px;background:${accent};color:#ffffff;text-align:center;font-size:9px;font-weight:bold;">+</span><strong>${rep(field.label || 'Thông tin')}:</strong> ${value}</div>`;
        });
        const detailHtml = [...details, ...customDetails].join('');
        const socials = socialLinks.length ? `<tr><td style="padding-top:10px;font-family:${fontFamily};font-size:${bodySize}px;line-height:1.4;">${socialLinks.map(([label, mark, urlKey]) => `<a href="${rep(normalizedSocialUrl(urlKey))}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin:0 8px 4px 0;color:${accent};text-decoration:none;font-weight:bold;"><span style="display:inline-block;min-width:18px;height:18px;line-height:18px;margin-right:4px;border-radius:9px;background:${accent};color:#ffffff;text-align:center;font-size:10px;">${mark}</span>${rep(urlKey === 'otherUrl' ? content.otherLabel || label : label)}</a>`).join('')}</td></tr>` : '';
        ...
        return `<table role="presentation" class="ft-email-block ft-email-signature-builder" width="${width}" border="0" cellspacing="0" cellpadding="0" style="width:100%;max-width:${width}px;border-collapse:collapse;border-top:5px solid ${accent};border-bottom:6px solid ${accent};margin-top:${marginTop}px;margin-bottom:${marginBottom}px;"><tr>${logo}<td valign="top" style="${divider}padding:${padding}px;font-family:${fontFamily};color:${text};"><div style="font-size:${titleSize}px;line-height:1.2;font-weight:700;letter-spacing:.2px;color:${accent};text-transform:uppercase;">${rep(content.fullName || '')}</div>${content.showJobTitle !== false && content.jobTitle ? `<div style="padding-top:5px;font-size:${Math.max(10, bodySize + 1)}px;line-height:1.4;color:${text};">${rep(content.jobTitle)}</div>` : ''}${content.showCompany !== false && content.company ? `<div style="padding-top:6px;font-size:${Math.max(10, bodySize + 1)}px;line-height:1.4;font-weight:700;color:${accent};text-transform:uppercase;">${rep(content.company)}</div>` : ''}${detailHtml || socials ? '<div style="margin:10px 0 9px;border-top:1px solid #d9e2ef;font-size:1px;line-height:1px;">&nbsp;</div>' : ''}<div style="font-size:${bodySize}px;line-height:1.65;color:${text};">${detailHtml}</div><table role="presentation" border="0" cellspacing="0" cellpadding="0" style="border-collapse:collapse;">${socials}</table></td></tr></table>`;
      }
      case 'signature': {
        const rawHtml = content.html || '';
        const fontSize = content.fontSize || 14;
