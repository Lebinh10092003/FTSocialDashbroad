...
export function importHtmlToEmailBlocks(html: string, seed = Date.now()): EmailHtmlBlockImportResult {
  ...
  const alignment = (element: Element) => {
    let current: Element | null = element;
    while (current && current !== documentNode.documentElement) {
      const raw = String((current as HTMLElement).style?.textAlign || '').trim() || current.getAttribute('align') || '';
      if (['left', 'center', 'right', 'justify'].includes(raw.toLowerCase())) return raw.toLowerCase();
      current = current.parentElement;
    }
    return 'left';
  };
  ...
  function convertElement(element: HTMLElement, depth = 0): EmailBlock[] {
    ...
    const generatedBlockType = element.getAttribute('data-ft-block-type');
    const generatedCell = element.querySelector<HTMLTableCellElement>(':scope > tbody > tr > td');
    // Keep the exact rich-text payload from emails generated by this editor.
    // Without this shortcut, nested <strong>/<a> elements were interpreted as
    // separate blocks and each subsequent HTML round-trip changed the layout.
    if (generatedBlockType === 'paragraph' && generatedCell) {
      return [{ ...paragraphBlock(generatedCell.innerHTML, generatedCell), styles: blockStyles(element) }];
    }
    if (generatedBlockType === 'section' && generatedCell) {
      return [{ ...sectionBlock(generatedCell, parseChildren(generatedCell, depth + 1)), styles: blockStyles(element, 0) }];
    }
    if (generatedBlockType === 'data-table' && generatedCell) {
      const dataTable = generatedCell.querySelector('table[role="table"]') as HTMLTableElement | null;
      if (dataTable) {
        const rows = Array.from(dataTable.rows).filter(row => row.closest('table') === dataTable);
        return [{ ...dataTableBlock(dataTable, rows), styles: blockStyles(element) }];
      }
    }
  ...
  const generatedContentTables = Array.from(documentNode.body.querySelectorAll<HTMLTableElement>('table.ft-email-content'));
  // HTML generated by this editor has a well-known content cell. On every
  // round-trip choose the innermost one: outer content shells can be present
  // when an old Custom HTML block already contains a previously exported mail.
  // Re-importing those shells caused each save to add another Section/card.
  const generatedContent = generatedContentTables.at(-1) || null;
  const generatedContentCell = generatedContent?.querySelector<HTMLTableCellElement>(':scope > tbody > tr > td.ft-email-content-cell') || null;
  if (generatedContentCell) {
    const blocks = parseChildren(generatedContentCell);
    return { blocks, customBlockCount, usedEmailContainer: true };
  }
