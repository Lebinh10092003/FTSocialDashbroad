  ...
  return (
    <div className="ft-module-shell flex h-screen overflow-hidden font-sans">
      <Sidebar
        ...
        onBackToWorkspace={() => setViewMode('workspace')}
      />
      <main className="flex-1 overflow-y-auto"><header className="ft-module-header sticky top-0 z-10 flex h-16 items-center justify-between border-b px-5 md:px-7"><div><p className="text-xs font-bold uppercase tracking-wide text-sky-700">Truyền thông</p><h1 className="text-base font-extrabold text-slate-900">{({dashboard:'Biểu đồ tổng quan',media:'Báo cáo tổng hợp',posts:'Bài đăng',sync:'Đồng bộ dữ liệu',config:'Cấu hình hệ thống'} as Record<string,string>)[activeTab]}</h1></div><span className="hidden rounded-full bg-sky-50 px-3 py-1.5 text-xs font-bold text-sky-800 sm:block">Fermat Workspace</span></header><nav className="ft-mobile-nav md:hidden" aria-label="Điều hướng Truyền thông"><button aria-current={activeTab==='dashboard'?'page':undefined} onClick={()=>setSocialTab('dashboard')}>Tổng quan</button><button aria-current={activeTab==='media'?'page':undefined} onClick={()=>setSocialTab('media')}>Báo cáo</button><button aria-current={activeTab==='posts'?'page':undefined} onClick={()=>setSocialTab('posts')}>Bài đăng</button><button aria-current={activeTab==='sync'?'page':undefined} onClick={()=>setSocialTab('sync')}>Đồng bộ</button>{!isGuest&&<button aria-current={activeTab==='config'?'page':undefined} onClick={()=>setSocialTab('config')}>Cấu hình</button>}</nav><div className="ft-module-content px-5 py-6 md:px-7 md:py-7">
        {loading ? (
